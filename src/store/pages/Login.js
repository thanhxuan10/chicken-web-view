...
class Login extends Component {
  render() {
    return (
      <div style={{ height: "80vh" }} className="d-flex flex-row justify-content-center align-item-center" color="light">
        <Col sm="10" md="6" lg="4" className="m-auto shadow-lg bg-white" >
          <div className="pr-4 pl-4 pb-3" color="danger" >
              ...
              {({ email, password, onChange, login, isLogin }) => {
                if (isLogin) {
                  ...
                  return <form>
                    ...
                    <Button className="w-100 m-1 " color="danger " onClick={login}>Login</Button>
                    <Button className="w-100 m-1" color="primary" onClick={() => { this.props.history.push('/auth/register') }}>Signup</Button>
                  </form>
                }
              }}
            </AuthContext.Consumer>
